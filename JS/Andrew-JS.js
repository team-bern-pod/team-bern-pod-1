...
function listOfRollsFromDieFunc(numberOfRolls, input) {
    ...
    for (var i = 0; i < numberOfRolls; i++) {
        diceArray.push(twelveSidedDie());
    }
    return diceArray
}
// console.log(numberOfRolls(4, rollDie()));

//this is the number of rolls picker
(function() {
    var diceArray = [];
    var numberOfRolls = document.getElementById('numberOfRolls');
    var diceToRoll = document.getElementById('diceToRoll');
    for (var i = 0; i < numberOfRolls; i++)
        return diceArray;
        diceArray.push(numberOfRolls());

    console.log(numberOfRolls); // prints <button id="numberOfRolls">Click Me</button>
    console.log(diceToRoll); // prints <button id="diceToRoll">Click Me</button>
